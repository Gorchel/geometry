 ...
 */
class KPSender implements SenderInterface
{
    /**
...
//        $documentsAttachment = array_merge($dealDocumentsAttachment, $objectDocumentsAttachment);
        ...
        if (!empty($object['type'])) {
            if ($object['type'] == Properties::RENT_TYPE) {
                ...
                $key = 'buy';
                $objParams['buy_price'] = !empty($objectCustoms['custom-72707']) ? strip_tags($objectCustoms['custom-72707']) : '';
                $objParams['buy_price'] = number_format((float)$objParams['buy_price'], 0, '.', ' ');
                $objParams['buy_price_meter'] = !empty($objectCustoms['custom-72709']) ? strip_tags($objectCustoms['custom-72709']) : '';
            ...
            $objParams['property_photo'] = !empty($objectCustoms['custom-87808']) ? strip_tags($objectCustoms['custom-87808']) : '';
            $objParams['traffic_video'] = !empty($objectCustoms['custom-65160']) ? strip_tags($objectCustoms['custom-65160']) : '';
            $objParams['review_video'] = !empty($objectCustoms['custom-65161']) ? strip_tags($objectCustoms['custom-65161']) : '';
            ...
            $objParams['description_additional'] = trim($objectAttributes['description']);
        }

        $kpLinks = KPHelper::makeLinks($actualObject);
        $objParams['kpLinks'] = $kpLinks;
